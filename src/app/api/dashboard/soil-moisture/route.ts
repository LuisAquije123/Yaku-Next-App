...
import { getSoilMoistureForCultivo } from '@/lib/dashboard/soil-moisture'
...
/**
 * GET /api/dashboard/soil-moisture?id_cultivo=1
 *
 * Devuelve el último registro de humedad de suelo para el cultivo especificado,
...
export async function GET(request: NextRequest) {
  ...
  try {
    const lectura = await getSoilMoistureForCultivo(idCultivo, session.userId)

    if (!lectura) {
      return NextResponse.json<SoilMoistureResponse>(
        {
          success: false,
    ...
    console.error('[GET /api/dashboard/soil-moisture]', error)
    return NextResponse.json<DashboardApiError>(
      {
        success: false,
        message: 'Error al obtener la humedad de suelo.',
      },
      { status: 500 },
    )
  }
