...
import { getSoilTemperatureForCultivo } from '@/lib/dashboard/soil-temperature'
import type { DashboardApiError } from '@/types/dashboard-api'

interface SoilTemperatureResponse {
  success: boolean
  data?: {
    valor: number | null
...
/**
 * GET /api/dashboard/soil-temperature?id_cultivo=1
 *
 * Devuelve el último registro de temperatura de suelo para el cultivo especificado,
...
export async function GET(request: NextRequest) {
  ...
  try {
    const lectura = await getSoilTemperatureForCultivo(idCultivo, session.userId)

    if (!lectura) {
      return NextResponse.json<SoilTemperatureResponse>(
        {
          success: false,
    ...
    console.error('[GET /api/dashboard/soil-temperature]', error)
    return NextResponse.json<DashboardApiError>(
      {
        success: false,
        message: 'Error al obtener la temperatura de suelo.',
      },
      { status: 500 },
    )
  }
