...
import { getAmbientTemperatureForCultivo } from '@/lib/dashboard/ambient-temperature'
import type { DashboardApiError } from '@/types/dashboard-api'

interface AmbientTemperatureResponse {
  success: boolean
  data?: {
    valor: number | null
...
/**
 * GET /api/dashboard/ambient-temperature?id_cultivo=1
 *
 * Devuelve el último registro de temperatura ambiente para el cultivo especificado,
...
export async function GET(request: NextRequest) {
  ...
  try {
    const lectura = await getAmbientTemperatureForCultivo(idCultivo, session.userId)

    if (!lectura) {
      return NextResponse.json<AmbientTemperatureResponse>(
        {
          success: false,
    ...
    console.error('[GET /api/dashboard/ambient-temperature]', error)
    return NextResponse.json<DashboardApiError>(
      {
        success: false,
        message: 'Error al obtener la temperatura ambiente.',
      },
      { status: 500 },
    )
  }
