...
export async function getHistoricoData(userId: number, idCultivo: number, dias: number): Promise<HistoricoResponse> {
  ...
  const cultivo = await prisma.cultivos.findFirst({
    ...
    select: {
      asignaciones_iot: {
        ...
        select: {
          id: true,
          componente: { select: { tipo_componente: { select: { nombre_modelo: true } } } },
          humedad_suelo: { where: { valido: true, fecha: { gte: fechaLimite } }, select: { valor: true, ema: true, fecha: true } },
          humedad_ambiente: { where: { valido: true, fecha: { gte: fechaLimite } }, select: { valor: true, ema: true, fecha: true } },
          temperatura_suelo: { where: { valido: true, fecha: { gte: fechaLimite } }, select: { valor: true, ema: true, temperatura: true, fecha: true } },
          temperatura_ambiente: { where: { valido: true, fecha: { gte: fechaLimite } }, select: { valor: true, ema: true, temperatura: true, fecha: true } }
        }
      }
    }
  });
  ...
  if (!cultivo) {
    return { chartData: [], stats: null, riegoLog: [] };
  }

  // Extraemos los IDs para hacer el match de los riegos
  const asignacionesIds = cultivo.asignaciones_iot.map(a => a.id);
  ...
  cultivo.asignaciones_iot.forEach(asig => {
    ...
    // (Agrupación de datos para el gráfico y métricas)
  ...
  // BÚSQUEDA DE RIEGOS
  const riegosRecientes = await prisma.riego.findMany({
    where: {
      estado: true,
      fecha: { gte: fechaLimite },
      id_asignacion: { in: asignacionesIds } // Hacemos match exacto con el array
    },
    orderBy: { fecha: 'desc' },
    take: 30,
    select: {
      id: true,
      fecha: true,
      cantidad_agua_litros: true,
      tipo_riego: true
    }
  });

  const riegoLog = riegosRecientes.map(r => {
    // Incrementamos el conteo en el gráfico macro
    const key = getLimaDateKey(r.fecha);
    if (dataPorDia.has(key)) dataPorDia.get(key).riegos++;

    let origenStr = 'Auto';
    let colorStr = '#22c55e'; 
    const tipo = (r.tipo_riego || '').toLowerCase();

    if (tipo.includes('manual')) { origenStr = 'Manual'; colorStr = '#f59e0b'; }
    else if (tipo.includes('ml')) { origenStr = 'ML'; colorStr = '#a855f7'; }

    const limaDate = new Date(r.fecha.toLocaleString('en-US', { timeZone: 'America/Lima' }));
    const dia = String(limaDate.getDate()).padStart(2, '0');
    const mes = String(limaDate.getMonth() + 1).padStart(2, '0');
    const hora = String(limaDate.getHours()).padStart(2, '0');
    const min = String(limaDate.getMinutes()).padStart(2, '0');

    return {
      id: r.id.toString(), // Convertimos BigInt a String para evitar errores en React
      fechaStr: `${dia}/${mes} ${hora}:${min}`,
      origen: origenStr,
      colorOrigen: colorStr,
      litros: r.cantidad_agua_litros ? Number(r.cantidad_agua_litros).toFixed(1) : '--'
    };
  });
  ...
  return { chartData, stats, riegoLog: riegoLog.slice(0, 20) };
}